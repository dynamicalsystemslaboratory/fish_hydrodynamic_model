...
%% speed - tunnel size relation
D_all = [3,10,15]*1e-2; % m; tunnel widths
U_all = [15.418333538045411, 15.769681628318356, 15.742480517023463]*1e-2; % swimming speeds

case_num = 3; 

D_tunnel = D_all(case_num); % m; tunnel width
U_swim = U_all(case_num); % m/s; swimming speed of fish used in simulation

%% read simulation results
fname = [simul_results_dir, 'simul_vel_opt_parah_', num2str(D_tunnel),'.mat'];
...
NL = 10+1;
% L_max = 1.2*L_fish; % 1.5*L_fish; % max length of sheet pair; make sure that the entire sheet pair is in the fluid domain
% L_sheet_all = linspace(0.00, L_max, NL);
L_sheet_all = linspace(0.027, 0.032, NL);
...
% xs_all = linspace(x_head, x_head+0.5*L_fish, Nxs); % coarse
xs_all = linspace(0.074, 0.077, Nxs); % fine

% separation distance
Ndist = 10+1;
% dist_all = linspace(0, TBA, Ndist); % coarse
dist_all = linspace(0.0016, 0.0019, Ndist); % fine
...
set(gca,'fontsize',FTsz);
view([1,-1,-1])
set(gcf,'position', [286   125   834   572]);
% box on
...
%% save data
if flag_save > 0
    fname = [save_dir, 'fit_sheet_para_parah_', num2str(D_tunnel),'.mat']; 
